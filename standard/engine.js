...
function cooking(rows){
 ...
 const columnC=(count<=25?C[count]:count<=40?15+count:25+.75*count)*1000;
 const high=eligible.some(r=>number(r.va)>12000);
 // Note 2: when ratings differ, ratings below 12 kW are treated as 12 kW for the average.
 const average=sum(eligible.map(r=>number(r.qty)*Math.max(12000,number(r.va))))/count;
 const increase=high?Math.floor((average-12000)/1000+.5):0;
 let demand=columnC*(1+.05*increase), method='Table 220.55, Column C'+(increase?' + '+(increase*5)+'%':'');
 if(eligible.every(r=>number(r.va)<=8750)){
  const groupA=eligible.filter(r=>number(r.va)<3500), groupB=eligible.filter(r=>number(r.va)>=3500);
  const na=sum(groupA.map(r=>number(r.qty))), nb=sum(groupB.map(r=>number(r.qty)));
  const fa=na<=25?A[na]:30, fb=nb<=25?B[nb]:nb<=30?24:nb<=40?22:nb<=50?20:nb<=60?18:16;
  const note3=sum(groupA.map(load))*fa/100+sum(groupB.map(load))*fb/100;
  if(note3<demand){demand=note3;method='Table 220.55, Note 3 (Columns A / B)';}
 }
 if(connected<demand){demand=connected;method='Nameplate at 100%';}
 return {total:full+demand,method:method+(full?' + other cooking at 100%':''),count};
}
...
function calculate(s){
 ...
 const continuous=sum((s.continuous||[]).filter(r=>load(r)>0).map(r=>number(r.qty)*(r.ev?Math.max(7200,number(r.va)):number(r.va))*(r.factor===1?1:1.25)));
